<template>
  <div>
    <section-definition title="Funcionários" event-update="atualizarFuncionario"></section-definition>
    <v-toolbar dark color="primary">
      <v-toolbar-title>Gerenciar Funcionários</v-toolbar-title>
      <v-divider
        class="mx-2"
        inset
        vertical
      ></v-divider>
      <v-spacer></v-spacer>
      <modal-lista-configuracao-ponto />
      <modal-cadastra-funcoes />
      <modal-cadastro-funcionario @funcionarioCadastrado="cadastroEfetuado = $event"></modal-cadastro-funcionario>
    </v-toolbar>
     <v-container fluid>
        ...
        <template slot="items" slot-scope="props">
          ...
          <td>
            <v-menu offset-y>
              ...
              <v-list>
                <v-list-tile alt="Editar" @click="editItem(props.item)">
                  <v-list-tile-title>Editar</v-list-tile-title>
                </v-list-tile>
                <v-divider></v-divider>
                <v-list-tile alt="Funções" @click="registrarFuncoes(props.item)">
                  <v-list-tile-title>Funções</v-list-tile-title>
                </v-list-tile>
                <v-divider></v-divider>
                <v-list-tile alt="Funções" @click="registrarConfiguracaoPonto(props.item)">
                  <v-list-tile-title>Configuração Ponto</v-list-tile-title>
                </v-list-tile>
              </v-list>
            </v-menu>
          </td>
...
export default {
  components: {
    ...
    CheckBoxState,
    ModalCadastraFuncoes,
    ModalListaConfiguracaoPonto
  },
  data() {
    return {
      dialog: false,
  ...
  created() {
    ...
    this.atualizar();
    EventBus.$on("atualizarFuncionario", () => {
      this.atualizar();
    });
  },
  methods: {

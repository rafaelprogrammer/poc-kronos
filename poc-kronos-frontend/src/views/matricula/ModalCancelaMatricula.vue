<template>
  <v-layout row justify-center>
    <v-dialog v-model="dialog" persistent width="80%">
      <v-btn class="hidden-sm-and-down" slot="activator" dark color="primary" alt="Cancelar Matrícula" @click="abrirCadastro()" title="Cancelar Matrícula"><v-icon>settings</v-icon>Registrar Cancelamento</v-btn>
      <v-card>
        <v-card-title>
        ...
        <v-card-text>
          <v-container grid-list-md>
            ...
            <v-layout wrap>
              ...
              <v-flex xs12 sm6 md3>
                <date-picker-custom label="Data Cancelamento" :v-model="matriculaCanceladaTransferida.dataInformada" @date="matriculaCanceladaTransferida.dataInformada = $event"></date-picker-custom>
              </v-flex>
            </v-layout>
            </v-form>
          </v-container>
        ...
        <v-card-actions>
          ...
          <v-btn color="primary" type="submit" v-if="matriculaCanceladaTransferida.dataInformada" @click.native="cancelar" dark>{{btnTitle}}</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-layout>
...
export default {
  ...
  methods: {
    async abrirCadastro() {
      ...
      this.formTitle = "Registrar Cancelamento";
      this.btnTitle = "Confirmar";
      this.dialog = true;
    },
    cancelar() {
      matriculaService.cancelar(this.matriculaCanceladaTransferida).then((dados) => {
        if(dados) {
          this.dialog = false;
          this.$emit("matriculaCancelada", true);
        }
      });
    },
    close() {

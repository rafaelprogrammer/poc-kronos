<template>
      <v-dialog v-model="dialog" persistent width="80%">
        <v-btn class="hidden-sm-and-down" v-if="botaoSeleciona" slot="activator" color="primary" @click="abrir()"><v-icon>filter_list</v-icon>Selecionar</v-btn>
        <v-card>
          <v-card-title>
...
export default {
  ...
  created() {
    ...
    EventBus.$on("abrirSelecionaPessoa", () => {
      this.abrir();
    });
  },
  methods: {

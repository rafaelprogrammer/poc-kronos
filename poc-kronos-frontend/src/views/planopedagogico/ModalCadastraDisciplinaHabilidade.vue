      ...
      <v-dialog v-model="dialog" persistent width="80%">
        ...
        <v-card>
          ...
          <v-card-text>
            <v-container grid-list-md>
                <v-layout align-center wrap fill-height>
                  <v-flex xs12 sm6 md3>
                    <date-picker-custom label="Data Início Vigência" :v-model="disciplinaHabilidade.dataInicioVigencia" @date="disciplinaHabilidade.dataInicioVigencia = $event" 
                    ></date-picker-custom>
                  </v-flex>
                  <v-flex xs12 sm6 md3>
                      <v-select v-model="disciplinaHabilidade.idSubFase" item-text="sigla" item-value="id" :items="subFases"
                      label="Bimestre">
                      </v-select>
                  </v-flex>
                </v-layout>
              <v-data-table v-model="selected" select-all :loading="loading" :headers="headers" :items="habilidades" class="elevation-1" :total-items="pagination.totalItems" :pagination.sync="pagination">
          ...
          <v-card-actions>
            ...
            <v-btn color="primary" type="submit" v-if="disciplinaHabilidade.dataInicioVigencia 
...
export default {
  name: "ModalCadastraDisciplinaHabilidade",
  props: {
    idDisciplina: Number,
    idComponente: Number,
    idPeriodo: Number,
  ...
  methods: {
    ...
    salvar() {
      this.disciplinaHabilidade.idsHabilidades = this.selected.map(s => s.id);
      this.disciplinaHabilidade.idDisciplina = this.idDisciplina;
      disciplinaService.salvarDisciplinaHabilidade(this.disciplinaHabilidade).then(() => {
        this.$emit("disciplinaHabilidadeCadastrada", true);
        this.dialog = false;
      });
    },
    ...
    async pesquisar() {
      if (this.liberaPesquisa) {
        this.loading = true;
        const paginacaoResultado = await baseCurricularService.listarHabilidadesParaInclusaoDisciplinasHabilidades(
          this.idDisciplina,
          this.idComponente,
    ...
    close() {
      this.dialog = false;
      setTimeout(() => {
        this.disciplinaHabilidade = new DisciplinaHabilidade();

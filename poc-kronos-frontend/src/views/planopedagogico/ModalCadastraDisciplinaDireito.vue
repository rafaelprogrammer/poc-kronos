<template>
      <v-dialog v-model="dialog" persistent width="80%">
        ...
        <v-card>
          <v-card-title>
            <span class="headline">Selecionar Direito</span>
          </v-card-title>
          <v-card-text>
            <v-container grid-list-md>
                <v-layout align-center wrap fill-height>
                  <v-flex xs12 sm6 md3>
                    <date-picker-custom label="Data Início Vigência" :v-model="disciplinaDireito.dataInicioVigencia" @date="disciplinaDireito.dataInicioVigencia = $event" 
                    ></date-picker-custom>
                  </v-flex>
                  <v-flex xs12 sm6 md3>
                      <v-select v-model="disciplinaDireito.idSubFase" item-text="sigla" item-value="id" :items="subFases"
                      label="Bimestre">
                      </v-select>
                  </v-flex>
                </v-layout>
              <v-data-table v-model="selected" select-all :loading="loading" :headers="headers" :items="direitos" class="elevation-1" :total-items="pagination.totalItems" :pagination.sync="pagination">
          ...
          <v-card-actions>
            ...
            <v-btn color="primary" type="submit" v-if="disciplinaDireito.dataInicioVigencia 
...
export default {
  name: "ModalCadastraDisciplinaDireito",
  props: {
    idDisciplina: Number,
    idNivel: Number,
    idPeriodo: Number,
  ...
  methods: {
    ...
    salvar() {
      this.disciplinaDireito.idsDireito = this.selected.map(s => s.id);
      this.disciplinaDireito.idDisciplina = this.idDisciplina;
      disciplinaService.salvarDisciplinaDireito(this.disciplinaDireito).then(() => {
        this.$emit("disciplinaDireitoCadastrada", true);
        this.dialog = false;
      });
    },
    ...
    async pesquisar() {
      if (this.liberaPesquisa) {
        this.loading = true;
        const paginacaoResultado = await baseCurricularService.listarDireitosParaInclusaoDisciplinasDireitos(
          this.idDisciplina,
          this.idNivel,
    ...
    close() {
      this.dialog = false;
      setTimeout(() => {
        this.disciplinaDireito = new DisciplinaDireito();

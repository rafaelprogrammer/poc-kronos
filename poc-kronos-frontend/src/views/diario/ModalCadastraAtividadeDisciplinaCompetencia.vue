          ...
          <v-card-text>
            <v-container grid-list-md>
              ...
              <v-data-table v-model="selected" 
                select-all
                :loading="loading" :headers="headers" :items="disciplinasCompetencias" class="elevation-1" :total-items="pagination.totalItems" :pagination.sync="pagination">
...
export default {
  name: "ModalCadastraAtividadeDisciplinaCompetencia",
  props: {
    idDisciplina: Number,
    idAtividade: Number,
    idSubFase: Number,
  ...
  methods: {
    ...
    salvar() {
      this.atividadeDisciplinaCompetencia.idsDisciplinaCompetencia = this.selected.map(s => s.id);
      this.atividadeDisciplinaCompetencia.idAtividade = this.idAtividade;
      atividadeService.salvarAtividadeDisciplinaCompetencia(this.atividadeDisciplinaCompetencia).then(() => {
        this.$emit("atividadeDisciplinaCompetenciaCadastrada", true);
        this.dialog = false;
      });
    },
    ...
    async pesquisar() {
      if (this.liberaPesquisa) {
        this.loading = true;
        const paginacaoResultado = await disciplinaService.listarParaAtividadeDisciplinaCompetencia(
          this.idDisciplina,
          this.idAtividade,
    ...
    close() {
      this.dialog = false;
      setTimeout(() => {
        this.atividadeDisciplinaCompetencia = new AtividadeDisciplinaCompetencia();

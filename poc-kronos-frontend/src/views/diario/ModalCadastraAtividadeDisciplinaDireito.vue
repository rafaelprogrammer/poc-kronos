        ...
        <v-card>
          ...
          <v-card-text>
            <v-container grid-list-md>
              ...
              <v-data-table v-model="selected" select-all :loading="loading" :headers="headers" :items="disciplinasDireitos" class="elevation-1" :total-items="pagination.totalItems" :pagination.sync="pagination">
...
export default {
  name: "ModalCadastraAtividadeDisciplinaDireito",
  props: {
    idDisciplina: Number,
    idAtividade: Number,
    idSubFase: Number,
  ...
  methods: {
    ...
    salvar() {
      this.atividadeDisciplinaDireito.idsDisciplinaDireito = this.selected.map(s => s.id);
      this.atividadeDisciplinaDireito.idAtividade = this.idAtividade;
      atividadeService.salvarAtividadeDisciplinaDireito(this.atividadeDisciplinaDireito).then(() => {
        this.$emit("atividadeDisciplinaDireitoCadastrada", true);
        this.dialog = false;
      });
    },
    ...
    async pesquisar() {
      if (this.liberaPesquisa) {
        this.loading = true;
        const paginacaoResultado = await disciplinaService.listarParaAtividadeDisciplinaDireito(
          this.idDisciplina,
          this.idAtividade,
    ...
    close() {
      this.dialog = false;
      setTimeout(() => {
        this.atividadeDisciplinaDireito = new AtividadeDisciplinaDireito();

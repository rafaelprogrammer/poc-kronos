            ...
            <v-container grid-list-md>
              <v-form @submit.prevent="pesquisar()" ref="form">
                <v-layout align-center wrap fill-height>
                    ...
                    <v-flex xs12 sm6 md2>
                      <v-text-field color="primary" label="Código" v-model="codigoHabilidade"></v-text-field>
                    </v-flex>
                    <v-flex xs12 sm6 md3>
                      <v-text-field color="primary" label="Descrição" v-model="descricaoHabilidade"></v-text-field>
                    </v-flex>
                    <v-flex xs12 sm6 md3>
...
export default {
  name: "ModalCadastraAtividadeDisciplinaHabilidade",
  props: {
    idDisciplina: Number,
    idAtividade: Number,
    idSubFase: Number,
  ...
  methods: {
    ...
    salvar() {
      this.atividadeDisciplinaHabilidade.idsDisciplinaHabilidade = this.selected.map(s => s.id);
      this.atividadeDisciplinaHabilidade.idAtividade = this.idAtividade;
      atividadeService.salvarAtividadeDisciplinaHabilidade(this.atividadeDisciplinaHabilidade).then(() => {
        this.$emit("atividadeDisciplinaHabilidadeCadastrada", true);
        this.dialog = false;
      });
    },
    ...
    limpar() {
      this.codigoHabilidade = null;
      this.descricaoHabilidade = null;
      this.pesquisar();
    },
    async pesquisar() {
      if (this.liberaPesquisa) {
        this.loading = true;
        const paginacaoResultado = await disciplinaService.listarParaAtividadeDisciplinaHabilidade(
          this.idDisciplina,
          this.idAtividade,
          this.idSubFaseFiltro,
          this.codigoHabilidade,
    ...
    close() {
      this.dialog = false;
      setTimeout(() => {
        this.atividadeDisciplinaHabilidade = new AtividadeDisciplinaHabilidade();

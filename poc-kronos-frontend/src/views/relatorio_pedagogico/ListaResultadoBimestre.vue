          ...
          <td>
            <v-menu offset-y>
              ...
              <v-list>
                ...
                <v-divider></v-divider>
                <v-list-tile alt="Frequência PDF" @click="gerarResultadoBimestrePdf(props.item)">
                  <v-list-tile-title>PDF</v-list-tile-title>
                </v-list-tile>
                <v-divider></v-divider>
                <v-list-tile alt="Frequência XLSX" @click="gerarResultadoBimestreXLSX(props.item)">
                  <v-list-tile-title>XLSX</v-list-tile-title>
                </v-list-tile>
                <v-list-tile alt="Atualizar" @click="atualizarResultadosBimestrais(props.item)">
...
export default {
  ...
  methods: {
    async atualizarResultadosBimestrais(disciplina) {
      await relatorioService.atualizarResultadosBimestrais(this.idTurma, this.ano, disciplina.id,
      disciplina.idFuncionario, this.idSubFaseExecucao);
      this.pesquisar();
    },
    gerarResultadoBimestrePdf(disciplina) {
      relatorioService.gerarResultadoBimestrePdf(
        disciplina,
    ...
    gerarResultadoBimestreXLSX(disciplina) {
      relatorioService.gerarResultadoBimestreXLSX(
        disciplina,
        this.idSubFaseExecucao,
        this.cursos.filter(c => c.id === this.idCurso)[0].nome,
        this.periodos.filter(p => p.id === this.idPeriodoExecucao)[0].sigla,
    ...
    async pesquisar() {
      if (this.idSubFaseExecucao) {
        this.loading = true;
        const paginacaoResultado = await relatorioService.listarPedagogicosResultadosBimestrais(
          this.idTurma,
          this.idSubFaseExecucao,

        ...
        <v-layout wrap> 
          ...
          <v-flex xs12 sm6 md2>
            <v-select v-model="idPeriodoExecucao" item-text="sigla" item-value="id"
              :items="periodos" label="Período"></v-select>
          </v-flex>
          <v-flex xs12 sm6 md12>
          ...
          <v-flex xs12 sm6 md2 v-if="idPeriodoExecucao && idsTiposSituacoesContratos.length > 0">
...
export default {
  name: "ListaAlunosPorPeriodoRSituacao",
  data() {
    return {
      dialog: false,
  ...
  methods: {
    gerarPdf() {
      this.loading = true;
      relatorioAlunosService.gerarPorPeriodoRSituacaoPDF(
    ...
    async pesquisar() {
      if (this.idPeriodoExecucao && this.idsTiposSituacoesContratos) {
        this.loading = true;
        const paginacaoResultado = await relatorioAlunosService.listarPorPeriodoRSituacoes(
          this.idPeriodoExecucao,
          this.idsTiposSituacoesContratos,
